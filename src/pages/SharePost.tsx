...
export default function Post() {
    ...
    useEffect(() => {
        ...
        .then(data => {
            const foundPost = data.data.find((item: Post) => item.shareId === Number(shareId))
            if (foundPost) {
            ...
            <SideBar handleWritePost={handleWritePost} />
                ...
                {post && (
                    <div className="px-[41px] py-[39px] w-[1191px] h-[655px] bg-gray-100 rounded-3xl">
                        <div>
                            <h1>{post.title}</h1>
                            <p className="my-[13px]">{post.userId} | {post.createDate} | 운동 루틴방</p>
                            <p>{post.contents}</p>
                        </div>
                    </div>
                )}
